...
class GroongaFileSearchService extends ServiceBase {
	...
	def search(shareId:String, pathPrefix:String, q:String, offset:Int, limit:Int):(Int, Seq[FileWithSnippets]) = {
        ...
        val cmd = ("select files --match_columns path_ft||name*10||contents*5 " +
          "--output_columns 'id,path,name,atime,ctime,mtime,size,sha1sum,updated_at,snippet_html(path_ft),snippet_html(name),snippet_html(contents)' " + 
